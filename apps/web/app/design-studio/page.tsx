...
export default async function DesignStudio({ searchParams }: { searchParams: Promise<{ workspace?: string; project?: string; requirement?: string; generated?: string; reviewed?: string }> }) {
  ...
  return <ProjectShell workspaceId={workspace.data} project={currentProject} activeSection="design-studio" breadcrumbs={[{ label: "Design Studio" }]}> 
    <section className="page-heading"><div><span className="eyebrow">PROMPT-TO-PROTOTYPE · HUMAN REVIEW</span><h1>Mô tả nhu cầu. Xem thiết kế. Tiếp tục tinh chỉnh.</h1><p>Chọn requirement làm bối cảnh, sau đó nhập prompt giống cách làm việc với một product designer.</p></div></section>
    ...
    {selectedId?.success && <form action={generateDesign} className="panel design-prompt"><span className="eyebrow">DESIGN PROMPT</span><h2>Bạn muốn giao diện hoạt động như thế nào?</h2><textarea name="instruction" maxLength={2000} placeholder="Ví dụ: Thiết kế dashboard tối giản, ưu tiên số liệu doanh thu, có bộ lọc thời gian và cảnh báo đơn hàng bất thường…" defaultValue={artifact?.instruction ?? ""}/>{hidden}<div><span className="muted">Kết quả được lưu thành proposal mới để bạn review.</span><button className="button primary" type="submit">Sinh thiết kế mới ✦</button></div></form>}
    {!preview ? <section className="empty-state"><h2>Chưa chọn yêu cầu.</h2><p>Design Studio chỉ tạo proposal; không sửa requirement gốc.</p></section> : <>
      <section className="panel design-governance"><div><span className="eyebrow">GOVERNED PROPOSAL</span><p>Requirement V{preview.requirementVersion} · {preview.generatorProfile}</p>{artifact && <><strong>{artifact.status} · Design V{artifact.artifactVersion} · {artifact.trigger}</strong>{artifact.instruction && <p>Prompt: {artifact.instruction}</p>}</>}</div>{artifact?.status === "PENDING_REVIEW" && <div className="review-actions"><form action={rejectDesign}>{hidden}<input type="hidden" name="artifact" value={artifact.id}/><input type="hidden" name="revision" value={artifact.revision}/><button className="button" type="submit">Từ chối</button></form><form action={acceptDesign}>{hidden}<input type="hidden" name="artifact" value={artifact.id}/><input type="hidden" name="revision" value={artifact.revision}/><button className="button primary" type="submit">Chấp nhận</button></form></div>}</section>
